...
決済システムを作るとき、私たちは自然に全体の文脈を見ます——カード、請求、返金、不正検知がひとつの流れとしてつながっています。しかし、コーディングエージェントは「Payment」を検索して`payment_service.py`だけを見つけます。同じトランザクションフローにある`fraud_detector.py`や`billing_engine.py`は、ファイル名に「payment」が含まれていないため見逃されます。

知識グラフがあれば変わります。`PaymentService` → `CardValidator` → `FraudDetector` → `BillingEngine`——コード間の関係をたどり、エージェントも全体の文脈を把握できるようになります。

hedwig-kgを使えば、コーディングエージェントはキーワードマッチングではなく、人間のようにつながりをたどって探索します。トークンは減り、結果は向上し、これまで見つけられなかったコードも発見できます。
