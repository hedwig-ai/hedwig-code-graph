...
Claude Codeで開発しているとき、何か物足りなさを感じたことはありませんか？

決済システムを作るとき、私たちは自然に全体の文脈を見ます——カード検証、返金ポリシー、PCIコンプライアンスがひとつのドメインです。しかし、コーディングエージェントに「返金ロジックを修正して」と頼むと、`refund_service.py`だけを見つけて終わります。返金上限が定義された`policy.yaml`、カード会社別ルールが書かれた`docs/payment-rules.md`、関連テスト`test_chargeback.py`——どれもファイル名に「refund」が含まれていないため、視野に入りません。

知識グラフがあれば変わります。コード、ドキュメント、設定、テストをキーワードではなくドメインの関係性で結びます。エージェントも全体の文脈を把握できるようになります。

hedwig-kgを使えば、コーディングエージェントはキーワードマッチングではなく、人間のようにファイルやレイヤーを横断して探索します。トークンは減り、結果は向上し、これまで見つけられなかったコードも発見できます。
