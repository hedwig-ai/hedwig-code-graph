...
Claude Codeで開発しているとき、何か違和感を感じたことはありませんか？

例えば、決済ドメインを開発するとき、私たちは自然にカード、Stripe、お金といった関連する概念を思い浮かべます。

しかし、コーディングエージェントにはそのつながりが見えません。「Payment」という単語を何度も検索するだけです。エージェントにとって「Payment」と「Card」はまったく別の単語なのです。

そのギャップを埋めるために、知識グラフを作成することで知識に意味を付与する製品を作りました。コーディングエージェントが人間のように考えられるようにするためです。

これを使えば、Claude Codeはより直感的にコードベースを探索し、トークンを節約し、今まで見つけられなかったコードも発見できるようになります。
