<p align="center">
  ...
  <p align="center">
    ...
    <br />
    <a href="#クイックスタート">クイックスタート</a> · <a href="#対応言語">言語</a> · <a href="#aiエージェント統合">統合</a> · <a href="#機能">機能</a> · <a href="../README.md">English</a> · <a href="README_ko.md">한국어</a> · <a href="README_zh.md">中文</a> · <a href="README_de.md">Deutsch</a>
  </p>
</p>
...
hedwig-cgはコード、ドキュメント、依存関係から統一されたコードグラフを構築します——コーディングエージェントがキーワード検索ではなく、プロジェクト全体を真に理解できるようになります。AST構造抽出とLLMセマンティック強化により、明示的な関係と隠れたクロスモジュール関係の両方を自動的に発見します。
...
AST抽出 + LLMセマンティック強化が自動的に実行されます — 構造的関係（import、呼び出し、継承）と隠れたセマンティック接続（デザインパターン、行動依存性、クロスモジュール関係）をワンパスで発見します。コードが変更されたら：
...
すべての検索クエリは5つの独立した検索シグナルを経て、重み付きRRFで単一のランキング結果に融合されます：

| シグナル | エンジン | 検索対象 |
|----------|----------|----------|
| **コードベクトル** | FAISS + `bge-small-en-v1.5` | 意味的に類似したコード（関数、クラス、メソッド） |
| **テキストベクトル** | FAISS + `multilingual-e5-small` | ドキュメント、コメント、マークダウン（100+言語） |
| **グラフ展開** | NetworkX重み付きBFS | 構造的に接続されたノード + INFERREDエッジ |
| **全文検索** | SQLite FTS5 + BM25 | ソースコード全体での正確なキーワードマッチ |
| **コミュニティコンテキスト** | Leiden階層クラスタリング | 同じ機能クラスタの関連ノード |

LLMセマンティック強化がグラフ展開とコミュニティシグナルを強化します — INFERREDエッジがASTだけでは接続できないノード間のパスを作成します。

### LLMセマンティック強化

AIコーディングエージェント内でビルドすると、エージェントのLLMがコードノードバッチを並列分析しINFERREDエッジを注入します — デザインパターン、行動依存性、クロスモジュール接続など静的解析では検出できない関係を発見します。別途APIキーは不要です。
