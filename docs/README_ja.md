...
cd your-project/
hedwig-cg opencode install    # OpenCodeの場合
# または: hedwig-cg claude install  # Claude Codeの場合
```

Claude CodeまたはOpenCodeに伝えてください：

> 「このプロジェクトのコードグラフをビルドして」

以上です。グラフがビルドされ、以降すべての検索で自動的に参照されます。セッション終了時にグラフが自動的にリビルドされます。
...
| **OpenCode** | `hedwig-cg opencode install` | `.opencode/skills/`にスキル |
...
| `opencode install\|uninstall` | OpenCode統合管理 |
