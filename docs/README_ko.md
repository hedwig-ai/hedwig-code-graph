...
모든 검색 쿼리는 5개의 독립적인 검색 신호를 거친 후 단일 랭킹 결과로 융합됩니다:

| 신호 | 엔진 | 찾는 것 |
|------|------|---------|
| **코드 벡터** | FAISS + `bge-small-en-v1.5` | 의미적으로 유사한 코드 (함수, 클래스, 메서드) |
| **텍스트 벡터** | FAISS + `multilingual-e5-small` | 문서, 주석, 마크다운 (100+ 언어) |
| **그래프 확장** | NetworkX 가중 BFS | 구조적으로 연결된 노드 (호출자, 피호출자, 임포트) |
| **전문 검색** | SQLite FTS5 + BM25 | 소스 코드 전체에서 정확한 키워드 매칭 |
| **커뮤니티 컨텍스트** | Leiden 클러스터링 | 같은 기능 클러스터의 관련 노드 |
| **RRF 융합** | 가중 역순위 융합 | 모든 신호를 결합 — 여러 신호에서 발견된 노드가 상위 랭크 |
