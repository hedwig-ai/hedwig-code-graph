...
cd your-project/
hedwig-cg opencode install    # OpenCode의 경우
# 또는: hedwig-cg claude install  # Claude Code의 경우
```

Claude Code 또는 OpenCode에게 말하세요:

> "이 프로젝트의 코드 그래프를 빌드해"

끝입니다. 그래프가 빌드되고 이후 모든 검색에서 자동으로 참조됩니다. 세션이 끝나면 그래프가 자동으로 리빌드됩니다.
...
| **OpenCode** | `hedwig-cg opencode install` | `.opencode/skills/`에 스킬 |
...
| `opencode install\|uninstall` | OpenCode 통합 관리 |
